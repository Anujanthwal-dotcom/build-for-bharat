...
export function SourceInspector({ sources, outline }: SourceInspectorProps) {
  ...
  if (collapsed) {
    return (
      <button
        ...
        className="flex h-full items-center gap-2 border-r border-white/[0.07] bg-neutral-950/90 px-2.5 text-zinc-400 transition-colors hover:text-white"
  ...
  return (
    <div className="flex h-full w-72 flex-col border-r border-white/[0.07] bg-neutral-950/90 backdrop-blur-xl">
        ...
        <CollapsibleList title="Extracted Outline" count={outline.length} defaultOpen>
          <ul className="space-y-1">
            {filteredOutline.map((item, index) => (
              <li
                key={`${item}-${index}`}
                className="flex items-center gap-2 rounded px-2 py-1 font-mono text-[11px] text-zinc-400 hover:bg-white/[0.04]"
              >
